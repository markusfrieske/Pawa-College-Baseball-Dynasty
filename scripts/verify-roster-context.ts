...
try {
  ...
  await pool.query("UPDATE players SET w_risp=50,pitch_sl=4,pitch_ch=7,pitch_2s=1,pitch_cb=2,pitch_ct=5,pitch_snk=3,abilities=$1 WHERE id='member-player'",[JSON.stringify(['Sangfroid'])]);
  ...
  for (const mode of ["simulated","reported"]) {
    ...
    for (const role of roles) {
      ...
      try {
        ...
        await expect(page.locator('.c9-card-pitches dt')).toHaveCount(role==='member'?7:1); checks++;
