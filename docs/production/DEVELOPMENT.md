...
Run `npm run test:report-errors` without a database to exercise the actual structured feedback component against issue shapes from the real validator. It uses the same installed-browser selection as the OCR gate (`PAWA_TEST_BROWSER_PATH` override; Edge on Windows), a fresh headless context and owned loopback fixture. It verifies all messages, accessible keyboard callbacks, safe fallback and retained synthetic host state. It does not exercise report-page navigation, real draft persistence or production CSS. See [W02 batch 03](W02_BATCH_03.md).

Run `npm run test:report-override` without a database for the actual commissioner-reason component in a fresh headless browser, using the same installed-browser selection as the other component gates. It verifies keyboard entry, required/length semantics and retained synthetic host state across phase remounts. It does not execute the report page. The expanded `test:reported-results` separately verifies server-derived role metadata, commissioner/co-commissioner and coaching authority, invalid-reason no-write snapshots, and bound submission audit details through real sessions and PostgreSQL. See [W02 batch 04](W02_BATCH_04.md).

Run `npm run test:report-entry-mode` without a database for the actual full/score-only radio control and shared payload builder. It uses the installed-browser selection of the other component gates and synthetic retained draft state; it does not execute the complete report page. The expanded `test:reported-results` now checks populated migration 0050 upgrades through the real runner/readiness gate, explicit null summary persistence, known/mixed summary preservation, score-only finalization and actual participant inbox records. The current required migration is `0054_postseason_coach_awards`; old values/defaults remain unchanged. See [W02 batch 05](W02_BATCH_05.md).

Build on a checkout containing the [required media assets](BUILD_PREREQUISITES.md) and run `npm run release:gate` in the configured release-test environment. That gate includes migration, production-access, real session, stale-cleanup, actual-entry-point, static-serving, reported-result, isolated OCR-review, report-feedback, commissioner-reason and entry-mode checks (with their explicit local database/role requirements), build, database integrations, browser tests, rest integration, and database invariants. Review skipped tests explicitly. An incomplete media checkout or unavailable test database must be recorded as an incomplete gate.
...
The advance-execution helper gate now includes the owned weekly coach reset, and the reported-results HTTP gate includes actual reset-stage takeover and inherited-readiness recovery. See [W03 batch 09](W03_BATCH_09.md). Other stages and fast-forward remain outside this reset guarantee.
...
## Durable game effects gate

The expanded `test:reported-results` also verifies migration 0053 without invented legacy contributions, coach rewards committed before any batch flush, concurrent different-game coach/standings/player totals, restart/replay, effect/rivalry fault rollback and effect-bearing league cleanup. It does not exercise the actual advance/checkpoint route or certify postseason milestone awards and chronological pitcher rest. See [W03 batch 04](W03_BATCH_04.md).

The expanded `test:reported-results` includes `scripts/verify-postseason-awards.ts`: populated migration 0054, legacy ambiguity, all milestone/perk deltas, concurrent/restarted/reassigned/no-coach replay, receipt rollback, restore refusal and cascade cleanup. The final milestone suite passed 771 assertions in both implementation and independent runs. It invokes the production award service through a fresh HTTP child process; it does not certify the actual advance/checkpoint route. See [W03 batch 05](W03_BATCH_05.md).

`test:reported-results` now includes `verify-postseason-bracket.ts` and `verify-postseason-advance.ts`. They cover identity-based full-season series winners, invalid-bracket no-write service rejections, real reported/force/quick-sim authority gates and actual checkpoint/award failure followed by an HTTP-process restart and retry. No operation or checkpoint rows are seeded for that drill. See [W03 batch 06](W03_BATCH_06.md); hard-kill/phase-flip/lease-takeover and standard-mode journey coverage remain separate gates.

`test:reported-results` also imports `verify-advance-recovery.ts`. It distinguishes seeded stale/invalid operation and handoff fixtures from actual checkpoint/completion/failure-status faults and an HTTP-child hard kill after phase flip. The kill fixture targets only its observed blocked PostgreSQL backend before releasing its test barrier; lease timestamps are advanced only in that synthetic league. The loopback database and child-process ownership checks still apply. See [W03 batch 07](W03_BATCH_07.md); it proves conservative recovery containment and exact-source inheritance, not automatic transition reconciliation or overlapping live-worker lease safety.
