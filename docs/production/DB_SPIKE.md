...
5. Run the numbered migration runner after schema push. Successful schema push alone does not prove readiness: numbered files add constraints, indexes and tables outside the shared schema, including the session store. Verify the current `EXPECTED_MIGRATION` from `server/lib/runMigrations.ts` is recorded (now `0050_report_unknown_summaries`) and `/health/ready` succeeds.
