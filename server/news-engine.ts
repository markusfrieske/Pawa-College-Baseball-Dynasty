...
export async function generateGameNewsArticles(
  ...
  const journalistRotation: Journalist[] = ["addie", "sully", "jimbo"];
  let journalistIndex = 0;
  let addieArticles = 0;
  let sullyArticles = 0;
  let jimboArticles = 0;
  ...
  for (const game of completedGames) {
    ...
    const currentJournalist = journalistRotation[journalistIndex % 3];
    journalistIndex++;

    if (isPostseason) {
      newsData = generatePostseasonGameNews(winner, loser, winnerScore, loserScore, phase, currentJournalist);
      await storage.createDynastyNews({
        leagueId,
        authorName: currentJournalist === "addie" ? ADDIE_NAME : currentJournalist === "sully" ? SULLY_NAME : JIMBO_NAME,
      ...
      if (currentJournalist === "addie") addieArticles++;
      else if (currentJournalist === "sully") sullyArticles++;
      else jimboArticles++;
    } else {
      const shouldReport = isUpset || (winnerScore - loserScore) >= 5 || (winnerScore - loserScore) <= 2 || Math.random() < 0.5;
  ...
  if (completedGames.length >= 3 && !phase.includes("offseason")) {
    ...
    const recapJournalist = addieArticles <= sullyArticles ? (addieArticles <= jimboArticles ? "addie" : "jimbo") : (sullyArticles <= jimboArticles ? "sully" : "jimbo");
    const recap = generateWeekRecapNews(recapResults, week, season, recapJournalist);
    if (recap) {
      await storage.createDynastyNews({
        leagueId,
        authorName: recapJournalist === "addie" ? ADDIE_NAME : recapJournalist === "sully" ? SULLY_NAME : JIMBO_NAME,
        title: recap.title,
        content: recap.content,
        category: "recap",
        journalist: recapJournalist,
        season,
        week,
        isAutoGenerated: true,
      });
  ...
  if (week % 4 === 0 && !phase.includes("offseason") && phase !== "conference_championship" && phase !== "super_regionals" && phase !== "cws") {
...
export async function generateRecruitCommitNewsArticle(
  ...
  const journalist = pick<Journalist>(["sully", "jimbo"]);
  await storage.createDynastyNews({
    leagueId,
    authorName: journalist === "sully" ? SULLY_NAME : JIMBO_NAME,
    title: newsData.title,
    content: newsData.content,
    category: "recruiting",
    journalist,
...
export async function generateConferenceUpdateNews(
  ...
  const journalists: Journalist[] = ["addie", "sully", "jimbo"];
  let confIndex = 0;
  ...
  for (const conf of conferences) {
    ...
    const journalist = journalists[confIndex % 3];
    confIndex++;
    ...
    if (newsData) {
      await storage.createDynastyNews({
        leagueId,
        authorName: journalist === "addie" ? ADDIE_NAME : journalist === "sully" ? SULLY_NAME : JIMBO_NAME,
        title: newsData.title,
        content: newsData.content,
        category: "conference",
        journalist,
