...
export const ARCHETYPE_DEFS: Record<Archetype, ArchetypeDefinition> = {
  ...
  late_bloomer: {
    ...
    events: [
      {
        id: "lb_1",
        scenePrompt: "Pixel art scene: lone infielder at 5am taking ground balls in an empty stadium with only the maintenance lights on, a single hunched scout in the stands writing frantically on a notepad nobody else saw, pre-dawn sky. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
        scenePromptPitcher: "Pixel art scene: lone pitcher at 5am throwing bullpen in an empty stadium with only the maintenance lights on, a single hunched scout in the stands writing frantically on a notepad nobody else saw, pre-dawn sky. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "lb_2",
        scenePrompt: "Pixel art scene: a scouting forum post on a glowing laptop screen titled 'The Most Undervalued Prospect Nobody Is Talking About,' twelve programs simultaneously reaching for phones in separate dugouts, the name at the top of the article. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "lb_3",
        scenePrompt: "Pixel art scene: phone screen lit up at midnight showing a batting cage video with 38 million view counter, notification storm of college logos and scout accounts flooding the comments, a shocked father's silhouette in the background. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
        scenePromptPitcher: "Pixel art scene: phone screen lit up at midnight showing a bullpen session video with 38 million view counter, notification storm of college logos and scout accounts flooding the comments, a shocked father's silhouette in the background. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  velocity_freak: {
    ...
    events: [
      {
        id: "vf_1",
        scenePrompt: "Pixel art scene: pitcher mid-delivery on a showcase mound, two radar guns in the foreground displaying error messages, three MLB scouts leaning forward with wide eyes in the upper deck, nine wild pitches chalked on a board behind the dugout. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "vf_2",
        scenePrompt: "Pixel art scene: sports science firm logo on a glass door, pitcher sitting in a biomechanics lab with sensors taped to his arm, two researchers in white coats staring at monitors with expressions of disbelief, a readout showing arm speed data off the charts. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "vf_3",
        scenePrompt: "Pixel art scene: state showcase stage at night, scoreboard flashing 104 mph, crowd of scouts erupting while the pitcher on the mound stands alone staring at his own hand in awe, rival coaches visibly defeated in the dugout across the field. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  swing_rebuild: {
    ...
    events: [
      {
        id: "sr_1",
        scenePrompt: "Pixel art scene: reclusive hitting guru in a dim private facility adjusting a teenager's stance at a tee, no windows, walls covered in handwritten notes and slow-motion freeze frames, a single lamp over the tee, outside world completely absent. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "sr_2",
        scenePrompt: "Pixel art scene: scouts in the front row leaping from their seats in slow-motion as a baseball screams on a line into the gap, one scout's notepad flying from his hands, another already dialing a phone, the batter's follow-through perfect and effortless. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  position_change: {
    ...
    events: [
      ...
      {
        id: "pc_2",
        scenePrompt: "Pixel art scene: catcher throwing to second base from his knees in a cloud of dust, catcher's coach standing frozen with a dropped clipboard, three scouts immediately turning to each other with open mouths, the stopwatch in one hand reading an impossible time. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  social_media_star: {
    ...
    events: [
      {
        id: "sm_1",
        scenePrompt: "Pixel art scene: player in a batting cage with ring lights, a phone on a tripod filming everything, notifications flooding the screen showing millions of views, Gatorade and Nike logos appearing in comments, the actual baseball game happening ignored in the background outside. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
        scenePromptPitcher: "Pixel art scene: pitcher in a bullpen with ring lights, a phone on a tripod filming everything, notifications flooding the screen showing millions of views, Gatorade and Nike logos appearing in comments, the actual game happening ignored in the background outside. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
        ...
        choiceB: "Call the high school coach back — build the alliance that has access to him", choiceBOutcome: "The coach becomes your inside relationship. He's worried too and appreciates that you treated this seriously.", choiceBWeights: W.safe_pos,
      ...
      {
        id: "sm_2",
        scenePrompt: "Pixel art scene: phone screen showing a screenshot of an old DM blowing up in viral shares, angry and defensive comments flooding in, the player sitting in the dark of his bedroom staring at the ceiling, a mother's hand visible placing a glass of water on the nightstand. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  confidence_crisis: {
    ...
    events: [
      ...
      },
      {
        ...
        choiceA: "Don't call. Write. Tell him what watching that meant — one coach to one person", choiceAOutcome: "The written word at the right moment becomes the thing he keeps. The relationship changes permanently.", choiceAWeights: W.bold_pos,
        choiceB: "Call him the next morning — just to check in, not to recruit", choiceBOutcome: "The call with no agenda lands exactly right. He talks for 40 minutes. You mostly listen.", choiceBWeights: W.safe_pos,
        choiceC: "Reach out to his mother first — tell her what you saw from the stands", choiceCOutcome: "The mother becomes your strongest ally. She calls her son that night and tells him what you said.", choiceCWeights: W.neutral_up,
      },
    ],
  },
  ...
  injury_risk: {
    ...
    events: [
      {
        id: "ir_1",
        scenePrompt: "Pixel art scene: pitcher in a recovery bullpen, a physical therapist watching every movement with hands raised ready to intervene, medical monitoring equipment on a nearby table, the pitcher's parents visible through a chain link fence gripping it with white knuckles, a radar gun reading 91 in the foreground. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "ir_2",
        scenePrompt: "Pixel art scene: pitcher mid-delivery frozen in time, one radar gun in the foreground reading 94 mph, the pitcher's face showing two expressions simultaneously — ecstasy and fear — hand reaching instinctively toward his elbow, catcher sprinting toward the mound. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
        ...
        choiceA: "Support whatever timeline he needs — tell him explicitly that patience is your plan", choiceAOutcome: "The unconditional timeline removes the pressure that has haunted every previous comeback attempt. Something visibly settles in him.", choiceAWeights: W.safe_pos,
        choiceB: "Offer specialized biomechanics consultation — give him elite resources, not just words", choiceBOutcome: "The specific medical offer translates care into action. It's the thing that makes the difference between feeling supported and being supported.", choiceBWeights: W.bold_pos,
        choiceC: "Reach out to encourage the aggressive timeline — showcase while the momentum is there", choiceCOutcome: "Pushing the timeline on an arm that has failed twice before is a risk he knows better than anyone. He hears the pressure. The relationship cools.", choiceCWeights: W.risky_neg,
      },
    ],
  },
  ...
  academic_concern: {
    ...
    events: [
      ...
      {
        id: "ac_2",
        scenePrompt: "Pixel art scene: player holding a grade report with a look of surprised satisfaction, a formal 4-page letter typed in legal brief style on the table beside it, a coach's phone screen showing three school names with data annotations, the letter ending with 'P.S. I will require academic latitude. I hope that is compatible.' Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  transfer_rumors: {
    ...
    events: [
      ...
      {
        id: "tr_2",
        scenePrompt: "Pixel art scene: viral sports article on a phone screen reading '[Player Name] to Sign with Rival Program as Early as This Week,' the recruit reading it alone on his bed looking more annoyed than sad, three coaches at separate tables simultaneously reading the same article in horror, a 'Published 4 minutes ago' timestamp. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  two_sport_athlete: {
    ...
    events: [
      {
        id: "ts_1",
        scenePrompt: "Pixel art scene: player standing at a literal fork in a road at dusk, one path lit by a baseball diamond, the other by a football stadium's glow, a phone in his hand with two different coaches' texts both reading 'call me when you're ready,' an AAU football coach's truck parked at the edge of the frame watching. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "ts_2",
        scenePrompt: "Pixel art scene: ESPN feature segment playing on a TV screen showing the player in a football helmet, baseball mentioned only in a scrolling footnote at the bottom, the player watching the segment alone in his room looking uncomfortable, phone in hand, unsent text to a baseball coach glowing on the screen. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
        ...
        choiceB: "Text back: 'Of course. Tomorrow morning, whenever you're up.' — give him space to set the terms", choiceBOutcome: "Letting him control the timing of a conversation he initiated shows respect for his pace. He calls at 7am.", choiceBWeights: W.safe_pos,
        choiceC: "Respond warmly but wait for him to schedule it — don't pounce on the opening", choiceCOutcome: "The measured response is respectful. The conversation happens on his terms and goes deeper than expected.", choiceCWeights: W.neutral_up,
      },
    ],
  },
    ...
    events: [
      {
        ...
        choiceB: "Connect him with a knuckleball pitcher who played at the professional level", choiceBOutcome: "Expert mentorship on this rare craft — from someone who's actually lived it — transforms his development trajectory.", choiceBWeights: W.bold_pos,
        choiceC: "Offer him the freedom to develop without the pressure of secondary pitch development", choiceCOutcome: "The promise of not being 'fixed' is the most valuable thing you can offer. He's been waiting for a program that gets it.", choiceCWeights: W.neutral_up,
      ...
      },
      {
        ...
        choiceB: "Reach out to MLB teams quietly — ask if there's a professional development interest", choiceBOutcome: "Showing a professional pipeline for his specific gift is the most exciting thing you can offer a pitcher no one has a template for.", choiceBWeights: W.bold_pos,
        choiceC: "Make the offer and let the results speak — stop analyzing and just commit", choiceCOutcome: "The offer without caveats or qualifiers is what a 9-0 pitcher with a 0.84 ERA deserves. He accepts before the call ends.", choiceCWeights: W.safe_pos,
      },
    ],
  },
  ...
  the_phenom: {
    ...
    events: [
      {
        id: "ph_1",
        scenePrompt: "Pixel art scene: 6am breaking news alert on every screen in a coach's office — incorrect story that the recruit has already committed — 40 other coaches frantically calling simultaneously, the recruit himself sitting in his kitchen with no phone visible, completely unaware, eating cereal. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "ph_2",
        scenePrompt: "Pixel art scene: elderly scout in stadium bleachers at sunset with binoculars and a jaw that won't close, a coaching staff member beside him frozen mid-sentence, the player on the field far below doing something that has no name yet, the whole stadium holding still. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "ph_3",
        scenePrompt: "Pixel art scene: commitment day — two school pennants on a table, a clock on the wall showing 58 minutes remaining, a mother on the phone with a coach, hands trembling slightly, the rest of the family watching her face for the answer. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  the_collapse: {
    ...
    events: [
      {
        id: "tc_1",
        scenePrompt: "Pixel art scene: a faded #1 ranking board with the player's name at the top but crossed out in shadow, the player's old program logo on the wall behind him distancing itself, whispers rendered as dark silhouettes leaning toward each other, the recruit sitting alone in a dugout corner while the world debates what happened to him. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "tc_2",
        scenePrompt: "Pixel art scene: player and coach in a dim locker room, single lamp overhead, amber light on both of them, no one else present, the player mid-sentence with the look of someone who has never said this out loud before, the coach completely still and listening. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "tc_3",
        scenePrompt: "Pixel art scene: player back on the field, scoreboard showing real numbers, former doubters visible as shadowy figures returning to the outfield fence, one coach's phone lighting up with a text from a rival school saying 'we want back in,' the player visible on the mound or at the plate, light breaking behind the stadium. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  the_two_sport_icon: {
    ...
    events: [
      ...
      {
        id: "tsi_3",
        scenePrompt: "Pixel art scene: pre-dawn phone call in a quiet room, player's face lit only by the phone screen, a handwritten note in front of him with 'baseball' circled twice and underlined, an agent's business card face-down on the desk beside him, the coach's voice the only sound in the room. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  the_scientist: {
    ...
    events: [
      ...
      {
        id: "sci_2",
        scenePrompt: "Pixel art scene: laptop screen showing a color-coded pre-call agenda with timestamps, topics like 'Spin Rate Development Philosophy (12 min)' and 'Exit Velocity Improvement Rate by Position, 5 years (18 min),' a coach's chair positioned in front of an empty call screen, the call timer already running, a sticky note reading 'Analytics coach is at wedding.' Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "sci_3",
        scenePrompt: "Pixel art scene: whiteboard with three school names and annotated data columns below each, a pen hovering over one of them, the room empty and quiet except for a single overhead light, the pen's shadow falling across the other two names as it moves decisively toward the third. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  folk_hero: {
    ...
    events: [
      ...
      {
        id: "fh_2",
        scenePrompt: "Pixel art scene: national TV satellite trucks filling the street outside a small-town home, neighbors watching from their porches and yards, the recruit standing in the front doorway looking at the circus that arrived overnight, a hand-written sign in someone's yard that reads 'Stay Home,' a coach's car parked two blocks away watching from a distance. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "fh_3",
        scenePrompt: "Pixel art scene: commitment announcement in a high school gymnasium with the bleachers overflowing — 800 people crammed in, another hundred watching through gym windows from outside — the player at a podium with tears visible, 600 members of his community behind him, a coach in the corner barely visible, also crying. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
  ...
  generational_prodigy: {
    ...
    events: [
      {
        id: "gp_1",
        scenePrompt: "Pixel art scene: 17-year-old in a dugout surrounded by a documentary camera crew, a nutritionist with a food container, and a manager on the phone — all of them there for him, none of them him — the player himself staring at the field with an expression of complete exhaustion, a Baseball America cover with his face visible in someone's hand in the background. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
      ...
      {
        id: "gp_3",
        scenePrompt: "Pixel art scene: 11pm phone call in a kitchen, entire family gathered at the table, the coach's face visible on a phone screen held by the father, the question hanging in the air like smoke, the player visible in the doorway listening but not entering, the manager's car visible through the kitchen window parked outside. Retro 16-bit SNES style, dark forest green background, gold accent lighting, no text.",
