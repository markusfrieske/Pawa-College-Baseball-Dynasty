...
async function generateRecruitFlip(ctx: DramaContext, team: Team) {
  ...
  await storage.createStoryEvent({
    ...
    description: `Sully's hearing that ${rivalTeam.name} is making an aggressive push for ${recruit.starRating}-star ${recruit.position} ${recruit.firstName} ${recruit.lastName} from ${recruit.hometown}, ${recruit.homeState}. They're offering a big NIL package. Your recruit's attention is being pulled. How do you respond?`,
    journalist: "sully",
    status: "pending",
    requiresChoice: true,
    choices: [
