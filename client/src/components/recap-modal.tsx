    ...
    <Dialog open={!!gameId} onOpenChange={(open) => { if (!open) onClose(); }}>
      ...
      >
        ...
        <div className="flex items-center gap-2 px-4 py-2.5 bg-gold/10 border-b border-gold/20">
          ...
          <span className="text-gold text-xs tracking-wide">GAME RECAP</span>
          {badges.length > 0 && (
            <div className="flex gap-1 ml-auto flex-wrap justify-end">
              {badges.map(b => {
                ...
                return (
                  <span
                    key={b}
                    className={`text-xs font-semibold px-1.5 py-0.5 rounded border ${cfg.className}`}
                  >
                    {cfg.label}
                  </span>
                );
          ...
          <div className="divide-y divide-border/30">
            ...
            <div className="px-4 pt-4 pb-3">
              <div className="flex items-center justify-between gap-3 mb-2">
                ...
                <div className="text-center flex-1 min-w-0">
                  ...
                  <p
                    className={`font-display text-3xl font-bold leading-none ${recap.awayScore > recap.homeScore ? "text-gold" : "text-foreground/60"}`}
                ...
                <div className="flex flex-col items-center flex-shrink-0">
                  <span className="text-xs font-semibold text-muted-foreground/60">FINAL</span>
                ...
                <div className="text-center flex-1 min-w-0">
                  ...
                  <p
                    className={`font-display text-3xl font-bold leading-none ${recap.homeScore > recap.awayScore ? "text-gold" : "text-foreground/60"}`}
              ...
              <div className="flex items-center justify-center gap-3 mt-2 flex-wrap">
                {recap.phase && (
                  <span className="text-xs font-semibold px-2 py-0.5 rounded bg-muted/50 text-muted-foreground uppercase">
                ...
                {recap.statsIncomplete && (
                  <span className="text-xs font-semibold text-yellow-600/80">STATS INCOMPLETE</span>
                )}
              </div>
            </div>
            ...
            {recap.lineScore && recap.lineScore.length > 0 && (
              <div className="px-3 py-3">
                <div className="flex items-center gap-1.5 mb-2">
                  <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Line Score</span>
                </div>
                <LineScoreTable
            ...
            {recap.playerOfGame && (
              <div className="px-4 py-3">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-xs font-semibold text-gold uppercase tracking-wider">Player of the Game</span>
                </div>
            ...
            {recap.turningPoint && (
              <div className="px-4 py-3">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-xs font-semibold text-amber-400/80 uppercase tracking-wider">Turning Point</span>
                </div>
            ...
            {recap.topHitters && (recap.topHitters as any[]).length > 0 && (
              <div className="px-4 py-3">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Top Performers</span>
                </div>
                  ...
                  {(recap.topHitters as any[]).map((hitter: any, i: number) => (
                    <div key={i} className="flex items-center justify-between gap-2 text-sm">
                      <div className="flex items-center gap-2 min-w-0">
                        <span
                          className="text-xs font-semibold px-1 py-0.5 rounded bg-muted/40 text-muted-foreground flex-shrink-0"
                        >
                          {hitter.teamAbbr}
                        </span>
            ...
            {recap.pitchingLine && (
              <div className="px-4 py-3">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Pitching Decision</span>
                </div>
                <div className="space-y-1.5">
                  {(recap.pitchingLine as any).winner && (
                    <div className="flex items-center justify-between gap-2 text-sm">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold text-emerald-400 border border-emerald-700/40 px-1 py-0.5 rounded">W</span>
                        <span className="text-foreground/90">{(recap.pitchingLine as any).winner.name}</span>
                        <span className="text-xs font-semibold text-muted-foreground">{(recap.pitchingLine as any).winner.teamAbbr}</span>
                      </div>
                  ...
                  {(recap.pitchingLine as any).loser && (
                    <div className="flex items-center justify-between gap-2 text-sm">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold text-red-400 border border-red-700/40 px-1 py-0.5 rounded">L</span>
                        <span className="text-foreground/90">{(recap.pitchingLine as any).loser.name}</span>
                        <span className="text-xs font-semibold text-muted-foreground">{(recap.pitchingLine as any).loser.teamAbbr}</span>
                      </div>
                      <span className="text-muted-foreground text-xs">
                  ...
                  {(recap.pitchingLine as any).save && (
                    <div className="flex items-center justify-between gap-2 text-sm">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold text-blue-400 border border-blue-700/40 px-1 py-0.5 rounded">SV</span>
                        <span className="text-foreground/90">{(recap.pitchingLine as any).save.name}</span>
                        <span className="text-xs font-semibold text-muted-foreground">{(recap.pitchingLine as any).save.teamAbbr}</span>
                      </div>
                      <span className="text-muted-foreground text-xs">
            ...
            {(recap.standingsImpact || recap.seriesStatus) && (
              <div className="px-4 py-3 space-y-2">
                ...
                {recap.seriesStatus && (
                  <div className="flex items-start gap-2 text-sm">
                    <span className="text-xs font-semibold text-muted-foreground mt-1 flex-shrink-0">SERIES</span>
