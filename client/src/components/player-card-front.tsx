...
export function PlayerCardFront({player,leagueId,teamColor}:{player:Player;leagueId?:string;teamColor?:string}) {
 ...
 return <article className="c9-card-front" data-testid="complete-player-card">
  ...
  {pitching&&<section className="c9-card-pitches"><h3>Pitch repertoire</h3><dl>{[...allPitchKeys,'SPL'].map(key=>{const value=player[('pitch'+key) as keyof Player];return <div key={key}><dt title={pitchLabels[key]||'Splitter'}>{key}</dt><dd>{value==null?'—':key==='FB'||key==='2S'?(Number(value)>0?'Equipped':'Absent'):`${value} / 7`}</dd></div>})}</dl></section>}
