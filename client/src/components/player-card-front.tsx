...
export function PlayerCardFront({player,leagueId,teamColor}:{player:Player;leagueId?:string;teamColor?:string}) {
 ...
 return <article className="c9-card-front" data-testid="complete-player-card">
  ...
  <div className="c9-card-bio"><span><Icon kind="person"/><b>Position / hand</b> {player.jerseyNumber == null ? '' : `#${player.jerseyNumber} · `}{player.position} · Bats {player.bats||player.batHand||'—'} / Throws {player.throws||player.throwHand||'—'}</span><span><Icon kind="class"/><b>Class</b> {player.eligibility||'Not recorded'}</span><span><Icon kind="home"/><b>Hometown</b> {hometown}</span></div>
