...
type ArrivalRecruit = RevealRecruit & Partial<Pick<Player, "jerseyNumber" | "eligibility" | "hairColor" | "hairStyle" | "facialHair" | "eyeStyle" | "eyebrowStyle" | "mouthStyle" | "eyeBlack">> & { signingDayRevealed?: boolean };
...
interface RevealData { league: { id: string; name: string; currentSeason: number }; teamData: TeamEntry[]; myTeamId: string | null }
...
export default function SigningDayRevealPage() {
  ...
  const query = useQuery<RevealData>({
  ...
  exportContext.current = `${leagueId}/${selectedKey}/${spotlight?.recruit.id ?? "class"}`;
  useEffect(() => { setTeamId(""); setSearch(""); setSpotlight(null); setPlaying(false); setSaveError(""); setNotice(""); setViewed(new Set()); setExportImage(null); }, [leagueId]);
  ...
  function openSpotlight(recruit: ArrivalRecruit) {
    ...
    setSpotlight({ recruit, team: selected.team, season: data.league.currentSeason });
  ...
  async function exportPNG(kind: "player" | "class") {
    ...
    try {
      ...
      setExportImage({ url: canvas.toDataURL("image/png"), filename: `class-of-nine-${slug}-season-${data?.league.currentSeason ?? ""}.png` });
    } catch (error) { setExportError(error instanceof Error ? error.message : "Image creation failed. Please retry."); }
    finally { setExporting(false); }
  }

  if (query.isPending) return <main className="c9-arrival-page"><p role="status">Opening the class record…</p></main>;
  if (query.isError) return <main className="c9-arrival-page"><h1>Class record unavailable</h1><p role="alert">{query.error.message}</p><button onClick={() => query.refetch()}>Retry class record</button><Link href={`/league/${leagueId}`}>Return to program</Link></main>;
  return <main className="c9-arrival-page">
    <header className="c9-arrival-heading"><div><span className="c9-arrival-kicker">{data?.league.name} / SEASON {data?.league.currentSeason}</span><h1>A new class. A place to belong.</h1><p>Choose the spotlight. Get to know the whole player.</p></div><Link href={`/league/${leagueId}`}>Return to program ↗</Link></header>
    <section className="c9-arrival-home"><img src={campus} alt="" className="c9-arrival-campus"/><div className="c9-arrival-home-copy"><img src="/brand/gateway.svg" alt="" className="c9-arrival-gateway"/><span className="c9-arrival-kicker">THE CLUBHOUSE / CLASS OF NINE</span><h2>{selected?.team.name ?? "The incoming class"}</h2><p>{recruits.length} committed {recruits.length === 1 ? "player" : "players"} · Season {data?.league.currentSeason}</p><p>Every player gets the same welcome. Open the class record for complete ratings and abilities.</p></div></section>
    ...
    <div ref={classSheet} className="c9-arrival-class-sheet">
      <div className="c9-arrival-class-title"><div><span className="c9-arrival-kicker">{selected?.team.name} / SEASON {data?.league.currentSeason}</span><h2>Remember these names.</h2></div><p>Committed class · {filtered.length}{filtered.length !== recruits.length ? ` of ${recruits.length} shown` : " players"}<br/>Alphabetical order</p></div>
      <div className="c9-arrival-gallery" data-testid="arrival-class-gallery">{filtered.map(r => <article className="c9-arrival-person" key={r.id}><Portrait recruit={r} color={selected?.team.primaryColor ?? "#234734"}/><div><span>{r.position} · {entryType(r)}</span><h3>{fullName(r)}</h3><p>{r.signingDayRevealed ? "Complete class record open" : "Awaiting class reveal"}</p><button data-html2canvas-ignore="true" onClick={() => openSpotlight(r)} disabled={!r.signingDayRevealed}>{viewed.has(r.id) ? "Replay spotlight" : "Meet this player"} <span aria-hidden="true">↗</span></button></div></article>)}</div>
      {!recruits.length && <p className="c9-arrival-empty">No commitments are recorded for this program. No ceremony is required.</p>}
    ...
    <p className="c9-arrival-footnote">Save a keepsake PNG to start your class scrapbook.</p>
