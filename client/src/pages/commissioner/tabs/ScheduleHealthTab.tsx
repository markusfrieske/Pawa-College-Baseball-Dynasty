...
function SeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <span className={`font-pixel text-[7px] px-1.5 py-0.5 rounded border ${cls} uppercase`}>
      {severity}
    </span>
  );
}
...
export function ScheduleHealthTab({ leagueId }: Props) {
  ...
  const tabBtnClass = (view: typeof activeView) =>
    `font-pixel text-[8px] px-3 py-1.5 rounded transition-colors ${
      activeView === view
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <RetroCard className="p-3">
          <p className="font-pixel text-[7px] text-muted-foreground mb-1">GAME COUNT RANGE</p>
          <p className="text-lg font-bold">
            {summary.minGames}–{summary.maxGames}
          </p>
          <p className="text-[10px] text-muted-foreground">target: {data.expectedGamesPerTeam}</p>
        </RetroCard>
        <RetroCard className="p-3">
          <p className="font-pixel text-[7px] text-muted-foreground mb-1">TOTAL BYE WEEKS</p>
          <p className={`text-lg font-bold ${summary.totalByes > data.numTeams ? "text-yellow-400" : ""}`}>
            {summary.totalByes}
          </p>
          <p className="text-[10px] text-muted-foreground">across {data.numTeams} teams</p>
        </RetroCard>
        <RetroCard className="p-3">
          <p className="font-pixel text-[7px] text-muted-foreground mb-1">HUMAN vs HUMAN</p>
        ...
        <RetroCard className="p-3">
          <p className="font-pixel text-[7px] text-muted-foreground mb-1">HEALTH STATUS</p>
          {summary.hasErrors ? (
          ...
          <p className="text-[10px] text-muted-foreground">{warnings.length} issue(s)</p>
        </RetroCard>
      </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                  ...
                  <span className="font-pixel text-[7px] text-muted-foreground">{w.code}</span>
                </div>
                <p className="text-[11px] text-foreground/90">{w.message}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex items-center gap-1 border-b border-border pb-2 overflow-x-auto">
        ...
        <button
          className="ml-auto font-pixel text-[7px] text-muted-foreground hover:text-foreground flex items-center gap-1"
          onClick={() => refetch()}
          disabled={isFetching}
      ...
      {activeView === "overview" && (
        <div className="space-y-3">
          <RetroCard>
            <RetroCardHeader>
              <span className="font-pixel text-[9px] text-gold">Schedule Configuration</span>
            </RetroCardHeader>
            <RetroCardContent className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
              <div>
                <p className="text-muted-foreground text-[10px]">Season Length</p>
                <p className="font-semibold capitalize">{data.seasonLength}</p>
              </div>
              <div>
                <p className="text-muted-foreground text-[10px]">Weeks</p>
                <p className="font-semibold">{data.numWeeks}</p>
              </div>
              <div>
                <p className="text-muted-foreground text-[10px]">Max Games/Week</p>
                <p className="font-semibold">{data.maxGamesPerWeek}</p>
              </div>
              <div>
                <p className="text-muted-foreground text-[10px]">Target Games/Team</p>
                <p className="font-semibold">{data.expectedGamesPerTeam}</p>
              </div>
              <div>
                <p className="text-muted-foreground text-[10px]">Teams</p>
                <p className="font-semibold">{data.numTeams}</p>
              </div>
              <div>
                <p className="text-muted-foreground text-[10px]">Season</p>
          ...
          <RetroCard>
            <RetroCardHeader>
              <span className="font-pixel text-[9px] text-gold">Game Count Distribution</span>
            </RetroCardHeader>
            <RetroCardContent className="space-y-2">
              <div className="grid grid-cols-3 gap-3 text-center text-xs mb-3">
                <div className="p-2 bg-muted/20 rounded">
                  <p className="font-pixel text-[7px] text-muted-foreground mb-1">MIN</p>
                  <p className={`text-lg font-bold ${summary.minGames < data.expectedGamesPerTeam * 0.85 ? "text-yellow-400" : ""}`}>
                    {summary.minGames}
                  </p>
                </div>
                <div className="p-2 bg-muted/20 rounded">
                  <p className="font-pixel text-[7px] text-muted-foreground mb-1">AVG</p>
                  <p className="text-lg font-bold">{summary.avgGames}</p>
                </div>
                <div className="p-2 bg-muted/20 rounded">
                  <p className="font-pixel text-[7px] text-muted-foreground mb-1">MAX</p>
              ...
              <div className="space-y-1.5">
                {teamStats
                  ...
                  .map(t => (
                    <div key={t.teamId} className="flex items-center gap-2 text-[10px]">
                      ...
                      />
                      {t.byeWeeks.length > 1 && (
                        <Badge variant="outline" className="font-pixel text-[6px] px-1 py-0 shrink-0 text-yellow-400 border-yellow-600/50">
      ...
      {activeView === "teams" && (
        <div className="space-y-2">
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {t.isHuman && (
                    <Badge variant="outline" className="font-pixel text-[6px] px-1.5 py-0 shrink-0 text-blue-400 border-blue-600/50">
                      HUMAN
                    </Badge>
                  )}
                ...
                {isExpanded && (
                  <div className="px-4 pb-3 space-y-2 border-t border-border/50 pt-2">
                    ...
                    {t.overloadedWeeks.length > 0 && (
                      <div className="p-2 rounded border border-red-500/30 bg-red-900/20 text-[10px]">
                        <span className="font-pixel text-[7px] text-red-400 mr-1">OVERLOADED:</span>
                    ...
                    {t.repeatOpponents.length > 0 && (
                      <div className="p-2 rounded border border-yellow-500/30 bg-yellow-900/20 text-[10px]">
                        <span className="font-pixel text-[7px] text-yellow-400 block mb-1">REPEAT OPPONENTS:</span>
          ...
          {teamStats.length > 12 && (
            <button
              className="w-full text-center font-pixel text-[8px] text-muted-foreground hover:text-foreground py-2"
      ...
      {activeView === "weeks" && (
        <RetroCard>
          <RetroCardHeader>
            <span className="font-pixel text-[9px] text-gold">Week-by-Week Breakdown</span>
          </RetroCardHeader>
          <RetroCardContent>
              ...
              <table className="w-full text-[10px]">
                <thead>
                  <tr className="border-b border-border text-muted-foreground">
                    <th className="text-left py-1.5 pr-3 font-pixel text-[7px]">WEEK</th>
                    <th className="text-right py-1.5 px-2 font-pixel text-[7px]">GAMES</th>
                    <th className="text-right py-1.5 px-2 font-pixel text-[7px]">CONF</th>
                    <th className="text-right py-1.5 px-2 font-pixel text-[7px]">OOC</th>
                    <th className="text-right py-1.5 px-2 font-pixel text-[7px]">ACTIVE</th>
                    <th className="text-right py-1.5 pl-2 font-pixel text-[7px]">BYES</th>
                  </tr>
                </thead>
                <tbody>
                  {weekStats.map(w => (
                ...
                <tfoot>
                  <tr className="border-t border-border text-muted-foreground">
                    <td className="py-1.5 pr-3 font-pixel text-[7px]">TOTAL</td>
      ...
      {activeView === "human" && humanTeamIds.length > 1 && (
        <RetroCard>
          <RetroCardHeader>
            <span className="font-pixel text-[9px] text-gold">Human vs. Human Matchup Matrix</span>
          </RetroCardHeader>
          <RetroCardContent>
            ...
            <div className="overflow-x-auto">
              <table className="text-[10px]">
                <thead>
                  <tr>
                    <th className="text-left py-1 pr-3 font-pixel text-[7px] text-muted-foreground" />
                    {humanTeamIds.map(tid => (
                      <th
                        key={tid}
                        className="text-center py-1 px-2 font-pixel text-[7px] text-muted-foreground max-w-[60px] truncate"
            ...
            <div className="flex items-center gap-4 mt-3 text-[9px] text-muted-foreground flex-wrap">
