...
- Implemented longer-term fix in this project: `leagues.isTestData` + `createdAt` columns, auto-flagged on create (explicit flag or name matching `/^e2e[\s-]/i`); a reusable `cleanupStaleLeagues()` (FK-ordered DELETE, never TRUNCATE, transaction-wrapped) purges test-flagged leagues after a few hours and guest-owned leagues after several days; runs both as a CLI script and a daily-guarded startup job. When adding new league-scoped tables, add them to this deletion order or bloat can recur silently.
