...
// Story Events (Drama) table - choice-based and automatic drama events
export const storyEvents = pgTable("story_events", {
  ...
  season: integer("season").notNull(),
  week: integer("week").notNull(),
  eventType: text("event_type").notNull(), // 'booster_pressure', 'recruit_flip', 'player_discipline', 'nil_negotiation', 'injury', 'chemistry', 'academic_probation', 'media_controversy'
  category: text("category").notNull().default("drama"), // 'drama', 'automatic'
  title: text("title").notNull(),
  description: text("description").notNull(),
  journalist: text("journalist"), // 'addie' or 'sully'
  ...
  choices: json("choices").$type<{ id: string; label: string; description: string; consequences: Record<string, any> }[]>().default([]),
  chosenOptionId: text("chosen_option_id"),
  consequences: json("consequences").$type<Record<string, any>>(),
  targetPlayerId: varchar("target_player_id").references(() => players.id),
  targetRecruitId: varchar("target_recruit_id").references(() => recruits.id),
  relatedTeamId: varchar("related_team_id").references(() => teams.id),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  resolvedAt: timestamp("resolved_at"),
});

export const insertStoryEventSchema = createInsertSchema(storyEvents).pick({
  leagueId: true,
  teamId: true,
  season: true,
  week: true,
...
// Story Arcs table - multi-week branching narrative arcs
export const storyArcs = pgTable("story_arcs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  leagueId: varchar("league_id").notNull().references(() => leagues.id),
  teamId: varchar("team_id").references(() => teams.id),
  arcType: text("arc_type").notNull(), // 'recruit_reveal', 'draft_watch', 'transfer_saga', 'rivalry', 'program_rise', 'breakout_player'
  title: text("title").notNull(),
  status: text("status").notNull().default("active"), // 'active', 'completed', 'abandoned'
  currentChapter: integer("current_chapter").notNull().default(1),
  totalChapters: integer("total_chapters").notNull().default(4),
  season: integer("season").notNull(),
  startWeek: integer("start_week").notNull(),
  targetRecruitId: varchar("target_recruit_id").references(() => recruits.id),
  targetPlayerId: varchar("target_player_id").references(() => players.id),
  metadata: json("metadata").$type<Record<string, any>>().default({}),
  revealedTraits: json("revealed_traits").$type<string[]>().default([]),
  outcome: text("outcome"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  completedAt: timestamp("completed_at"),
});

export const insertStoryArcSchema = createInsertSchema(storyArcs).pick({
  leagueId: true,
  teamId: true,
  arcType: true,
  title: true,
...
// Story Arc Chapters - individual beats within a story arc
export const storyArcChapters = pgTable("story_arc_chapters", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  arcId: varchar("arc_id").notNull().references(() => storyArcs.id),
  chapterNumber: integer("chapter_number").notNull(),
  title: text("title").notNull(),
  content: text("content").notNull(),
  journalist: text("journalist").notNull(), // 'addie' or 'sully'
  triggerWeek: integer("trigger_week").notNull(),
  triggerSeason: integer("trigger_season").notNull(),
  status: text("status").notNull().default("pending"), // 'pending', 'triggered', 'skipped'
  traitRevealed: text("trait_revealed"),
  traitType: text("trait_type"), // 'ability', 'personality', 'gem_bust', 'attribute'
  traitValue: text("trait_value"),
  branchCondition: text("branch_condition"), // JSON condition for when this chapter triggers
  branchOutcomes: json("branch_outcomes").$type<{ condition: string; nextChapter: number; description: string }[]>().default([]),
  requiresAction: boolean("requires_action").notNull().default(false),
  actionTaken: boolean("action_taken").notNull().default(false),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});

export const insertStoryArcChapterSchema = createInsertSchema(storyArcChapters).pick({
  arcId: true,
  chapterNumber: true,
  title: true,
  content: true,
...
export type InsertStoryArcChapter = z.infer<typeof insertStoryArcChapterSchema>;
export type StoryArcChapter = typeof storyArcChapters.$inferSelect;

// Moments table - milestone achievements
export const moments = pgTable("moments", {
  ...
  season: integer("season").notNull(),
  week: integer("week"),
  momentType: text("moment_type").notNull(), // 'program_first', 'player_record', 'coach_milestone', 'dynasty_achievement'
  category: text("category").notNull(), // 'first_conf_title', 'first_cws', 'win_milestone', 'cinderella_run', etc.
  title: text("title").notNull(),
  description: text("description").notNull(),
  journalist: text("journalist"),
  targetPlayerId: varchar("target_player_id").references(() => players.id),
  targetCoachId: varchar("target_coach_id").references(() => coaches.id),
  metadata: json("metadata").$type<Record<string, any>>().default({}),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});
...
export const storyEventsRelations = relations(storyEvents, ({ one }) => ({
  ...
  team: one(teams, { fields: [storyEvents.teamId], references: [teams.id] }),
  targetPlayer: one(players, { fields: [storyEvents.targetPlayerId], references: [players.id] }),
  targetRecruit: one(recruits, { fields: [storyEvents.targetRecruitId], references: [recruits.id] }),
}));

export const storyArcsRelations = relations(storyArcs, ({ one, many }) => ({
  league: one(leagues, { fields: [storyArcs.leagueId], references: [leagues.id] }),
  team: one(teams, { fields: [storyArcs.teamId], references: [teams.id] }),
  targetRecruit: one(recruits, { fields: [storyArcs.targetRecruitId], references: [recruits.id] }),
  targetPlayer: one(players, { fields: [storyArcs.targetPlayerId], references: [players.id] }),
  chapters: many(storyArcChapters),
}));

export const storyArcChaptersRelations = relations(storyArcChapters, ({ one }) => ({
  arc: one(storyArcs, { fields: [storyArcChapters.arcId], references: [storyArcs.id] }),
}));

export const momentsRelations = relations(moments, ({ one }) => ({
  league: one(leagues, { fields: [moments.leagueId], references: [leagues.id] }),
  team: one(teams, { fields: [moments.teamId], references: [teams.id] }),
  targetPlayer: one(players, { fields: [moments.targetPlayerId], references: [players.id] }),
  targetCoach: one(coaches, { fields: [moments.targetCoachId], references: [coaches.id] }),
}));
